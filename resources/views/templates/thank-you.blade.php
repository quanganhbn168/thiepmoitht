{{-- Template Name: Trang thư cảm ơn --}}
{{-- Template Type: reunion_thank_you --}}
...
@section('content')
    @php
        $heroImage = $event->hero_image ?? $event->background ?? $event->share_image;
        $mainUrl = $event->main_url ?? url('/' . $reunion->slug);
        $thankUrl = $event->thank_you_url ?? url('/' . $event->slug);
        $contacts = collect($organizers ?? [])->filter(fn ($item) => !empty($item['name']) || !empty($item['phone']));
        $recipient = $event->thank_you_recipient ?? null;
        $recipients = collect($event->thank_you_recipients ?? []);
        $recipientName = $recipient['name'] ?? 'Quý mạnh thường quân';
    @endphp
            ...
            <div class="mx-auto grid min-h-[86vh] max-w-6xl gap-10 px-5 py-10 sm:px-8 lg:grid-cols-[minmax(0,1fr)_390px] lg:items-center lg:py-16">
                <div class="pt-14 lg:pt-0">
                    ...
                    <p class="mt-6 max-w-2xl text-lg leading-8 text-stone-100 sm:text-xl">
                        Ban tổ chức trân trọng cảm ơn {{ $recipientName }} đã đồng hành, sẻ chia và góp sức để ngày hội ngộ thêm trọn vẹn.
                    </p>
                ...
                <aside class="rounded-lg border border-white/15 bg-white/12 p-5 shadow-2xl backdrop-blur-md sm:p-6">
                    <div class="rounded-md bg-[#fffaf0] p-5 text-stone-900 shadow-xl">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                <p class="text-xs font-bold uppercase tracking-[0.18em] text-stone-500">Tri ân đồng hành</p>
                                <h2 class="mt-1 font-serif text-2xl font-bold">{{ $recipientName }}</h2>
                                @if(!empty($recipient['role']))
                                    <p class="mt-1 text-sm text-stone-600">{{ $recipient['role'] }}</p>
                                @endif
                            </div>
                        </div>
            ...
            <div class="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[300px_minmax(0,1fr)]">
                <aside class="lg:pt-10">
                    ...
                    <h2 class="mt-3 font-serif text-3xl font-bold leading-tight sm:text-4xl">
                        Những đóng góp đã làm nên một ngày trở về ấm áp.
                    </h2>
                    @if(!empty($event->greeting))
                        <p class="mt-6 rounded-md border border-[#d6c2a3] bg-white/60 px-4 py-3 text-sm font-semibold text-[#31584b]">
                            {{ $recipient ? 'Kính gửi ' . $recipientName : $event->greeting }}
                        </p>
                    @endif
                </aside>
                ...
                <article class="thank-paper rounded-lg border border-[#dfd0b8] bg-[#fffaf0] p-6 shadow-[0_24px_80px_rgba(75,54,33,.12)] sm:p-10">
                    <div class="mb-8 flex items-center justify-between gap-4 border-b border-[#dfd0b8] pb-6">
                        <div>
                            ...
                            <h3 class="mt-2 font-serif text-3xl font-bold">{{ $recipientName }}</h3>
                            @if(!empty($recipient['role']))
                                <p class="mt-1 text-sm font-semibold text-[#31584b]">{{ $recipient['role'] }}</p>
                            @endif
                        </div>
                    ...
                    @if(!empty($recipient['note']))
                        <div class="mt-8 rounded-md border border-[#d6c2a3] bg-[#f8efdf] p-5">
                            ...
                            <p class="mt-2 leading-7 text-stone-700">{{ $recipient['note'] }}</p>
                        </div>
                    @endif
                    ...
                    <div class="mt-10 grid gap-4 border-t border-[#dfd0b8] pt-6 sm:grid-cols-3">
                        <div class="rounded-md bg-[#f4ead9] p-4">
                            ...
                            <p class="mt-1 text-sm leading-6 text-stone-600">Góp sức để chương trình được chuẩn bị chu đáo hơn.</p>
                        </div>
                        ...
                        <div class="rounded-md bg-[#f7efe7] p-4">
                            ...
                            <p class="mt-1 text-sm leading-6 text-stone-600">Ban tổ chức trân trọng lưu giữ và biết ơn từng tấm lòng.</p>
                        </div>
                    </div>
        ...
        @if($recipients->isNotEmpty())
            <section class="px-5 pb-16 sm:px-8 sm:pb-20">
                <div class="mx-auto max-w-6xl">
                    <div class="mb-7">
                        <p class="text-xs font-bold uppercase tracking-[0.24em] text-[#8b5d2c]">Danh sách tri ân</p>
                    ...
                    <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach($recipients as $item)
                            @php
                                $itemUrl = url('/' . $reunion->slug . '/thu-cam-on/' . $item['code']);
                                $isActive = !empty($recipient['code']) && $recipient['code'] === $item['code'];
                            @endphp
                            <a href="{{ $itemUrl }}" class="block rounded-lg border {{ $isActive ? 'border-[#31584b] bg-[#eef3ef]' : 'border-[#dfd0b8] bg-[#fffaf0]' }} p-5 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
                                <p class="font-bold text-stone-950">{{ $item['name'] }}</p>
                                @if(!empty($item['role']))
                                    <p class="mt-1 text-sm font-semibold text-[#8b5d2c]">{{ $item['role'] }}</p>
                                @endif
                                @if(!empty($item['note']))
                                    <p class="mt-3 line-clamp-2 text-sm leading-6 text-stone-600">{{ $item['note'] }}</p>
                                @endif
                                <p class="mt-4 text-xs font-bold uppercase tracking-[0.16em] text-[#31584b]">Xem thư riêng</p>
                            </a>
                        @endforeach
                    </div>
                </div>
